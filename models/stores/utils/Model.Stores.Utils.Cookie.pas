...
type
  DecodeFunction = function(const Val: string): string;

  SerializeOptions = record
  ...
  ParseOptions = class
  private
    FDecode: DecodeFunction;
  public
    property Decode: DecodeFunction read FDecode write FDecode;
  end;

function CookieParse(const Str: string; Options: ParseOptions = nil): TDictionary<string, string>;
function CookieSerialize(name, Val: string; Options: SerializeOptions): string;
function DefaultDecode(const Val: string): string;
function DefaultEncode(const Val: string): string;
...
function DefaultEncode(const Val: string): string;
begin
  //Result := TNetEncoding.URL.Encode(Val);
end;
